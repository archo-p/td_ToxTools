...
class ToxToolsExt:
	...
	def SaveTox(self, comp, path, doBackup = False, backupInfo = {'date':True, 'suffix':None}, enableToeBackup=False):
		...
		# a shitty stopgap to stop from making a defaulted path format and filename
		formatSavePath = True
	...
	def ExternalizeComp(self, comp, pathInfo = {'pathType':'choose', 'makeToxFolder':True}, backupInfo = {'date':True, 'suffix':None}, doVersion = True, enableToeBackup = False, relativeToxPath = True, updateToxPathPar = True, saveBackups = True):
		...
		if pathInfo != None:
			...
			savePath = None
			self.SaveTox(comp, savePath, doBackup=saveBackups)
	...
	def SaveSelected(self):
		if self.SelectedCompsTable.numRows > 0:
			for item in self.SelectedCompsTable.rows():
				...
				doBackups = manager.par.Savebackups.eval()
				self.ExternalizeComp(compToSave, pathInfo=None, saveBackups = doBackups)
